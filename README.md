...
and attention boundaries. `embeddinggemma_embed()` requires an explicit
task for other uses. Set `dimensions` to 512, 256, or 128 for Matryoshka
...
normalization, and MaxSim contract. `colbert_rank()` is exact candidate
scoring; use a dedicated late-interaction index for large corpora. Set
`COLBERT_WEIGHTS_FILE` to a local `LFM2.5-ColBERT-350M` GGUF to execute
this example when rendering the README.
