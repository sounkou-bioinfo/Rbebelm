...
old_max_gen <- Sys.getenv("BEBELM_AGENT_MAX_GEN", unset = NA_character_)
old_max_think <- Sys.getenv("BEBELM_AGENT_MAX_THINK", unset = NA_character_)
Sys.setenv(BEBELM_NUM_THREADS = "2", BEBELM_AGENT_MAX_GEN = "256", BEBELM_AGENT_MAX_THINK = "48")
threads_default <- eval(formals(bebel_r_agent_start)$num_threads)
max_gen_default <- eval(formals(bebel_r_agent_start)$max_gen)
max_think_default <- eval(formals(bebel_r_agent_start)$max_think)
if (is.na(old_threads)) Sys.unsetenv("BEBELM_NUM_THREADS") else Sys.setenv(BEBELM_NUM_THREADS = old_threads)
if (is.na(old_max_gen)) Sys.unsetenv("BEBELM_AGENT_MAX_GEN") else Sys.setenv(BEBELM_AGENT_MAX_GEN = old_max_gen)
if (is.na(old_max_think)) Sys.unsetenv("BEBELM_AGENT_MAX_THINK") else Sys.setenv(BEBELM_AGENT_MAX_THINK = old_max_think)
expect_true(is.double(threads_default))
expect_true(is.double(max_gen_default))
expect_true(is.double(max_think_default))
