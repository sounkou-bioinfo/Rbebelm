...
Async jobs use an aio-style surface: submit work, poll the handle, then collect
the completed `Turn` on the R thread. Several jobs can share one loaded model;
the weights stay shared and model execution is serialized until the event
monitor queue lands. Agent async jobs currently run on a cloned agent snapshot:
