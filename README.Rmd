...
Async jobs run generation on Rust threads and collect a finished `Turn` back on
the R thread. They do not invoke R callbacks while running. Agent async jobs run
on a cloned agent snapshot: the original agent is not mutated, but the model
weights are shared.
...
This is not a model-quality benchmark. It is a reproducible package-level smoke
benchmark that records prompt size, generation size, and throughput for a few
deterministic tasks.
