...
object that `r_objects()` can later see. The `r_eval` tool is only advertised to
...
A compact one-liner keeps the loaded BebeLM model object out of `.GlobalEnv`
while still sharing `.GlobalEnv` with `/r` and the agent's R tools:
