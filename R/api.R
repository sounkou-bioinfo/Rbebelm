...
#'
#' @param path Path to an `LFM2.5-ColBERT-350M` GGUF file. The supported
#'   reference artifact is `LFM2.5-ColBERT-350M-Q4_K_M.gguf` from
#'   `LiquidAI/LFM2.5-ColBERT-350M-GGUF`.
#' @param num_threads Optional Rayon global thread-pool size. This can only be
#'   set once per R process.
#' @return A `ColbertModel` object.
#' @export
colbert_model_load <- function(path, num_threads = NULL) {
  options <- ColbertModelLoadOptions(
    path = path,
    num_threads = if (is.null(num_threads)) NULL else as.numeric(num_threads)
  )
  ColbertModel$load(
...
#'
#' @param model A `ColbertModel` object.
#' @return A named list describing its token-vector and MaxSim contract.
#' @export
colbert_model_info <- function(model) {
  model <- S7::prop(ColbertModelRef(value = list(model)), "value")[[1L]]
  model$info()
}
...
#'
#' @param model A `ColbertModel` object.
#' @param text A non-empty query string.
#' @return A query `ColbertEmbeddings` object.
#' @export
colbert_encode_query <- function(model, text) {
  model <- S7::prop(ColbertModelRef(value = list(model)), "value")[[1L]]
...
#'
#' @param model A `ColbertModel` object.
#' @param text A non-empty document string.
#' @return A document `ColbertEmbeddings` object.
#' @export
colbert_encode_document <- function(model, text) {
  model <- S7::prop(ColbertModelRef(value = list(model)), "value")[[1L]]
  text <- S7::prop(BebelScalarText(value = text), "value")
  model$encode_document(text)
}

#' Inspect ColBERT token vectors
#'
#' @param embeddings A `ColbertEmbeddings` object.
#' @return A named list containing role, token count, and dimensions.
#' @export
colbert_embeddings_info <- function(embeddings) {
  embeddings <- S7::prop(ColbertEmbeddingsRef(value = list(embeddings)), "value")[[1L]]
  embeddings$info()
}
...
#'
#' @param embeddings A `ColbertEmbeddings` object.
#' @return Integer token ids, one per vector row.
#' @export
colbert_embedding_ids <- function(embeddings) {
  embeddings <- S7::prop(ColbertEmbeddingsRef(value = list(embeddings)), "value")[[1L]]
  embeddings$ids()
}
...
#'
#' @param embeddings A `ColbertEmbeddings` object.
#' @return An `n_tokens × 128` numeric matrix whose rows have unit L2 norm.
#' @export
colbert_embedding_vectors <- function(embeddings) {
  embeddings <- S7::prop(ColbertEmbeddingsRef(value = list(embeddings)), "value")[[1L]]
  embeddings$vectors()
}
...
#'
#' @param query Query `ColbertEmbeddings`.
#' @param document Document `ColbertEmbeddings`.
#' @return A numeric MaxSim relevance score.
#' @export
colbert_maxsim <- function(query, document) {
  query <- S7::prop(ColbertEmbeddingsRef(value = list(query)), "value")[[1L]]
  document <- S7::prop(ColbertEmbeddingsRef(value = list(document)), "value")[[1L]]
  query$maxsim(document)
}

#' Rank documents with ColBERT MaxSim
#'
#' This convenience helper performs exact in-memory scoring. It is suitable for
#' a candidate set; production-scale retrieval needs an external late-
#' interaction index built with the same profile and preprocessing contract.
#'
#' @param model A `ColbertModel` object.
#' @param query A non-empty query string.
#' @param documents A non-empty character vector of candidate documents.
#' @return A decreasing named numeric vector of MaxSim scores with class
#'   `colbertRanking`.
#' @export
colbert_rank <- function(model, query, documents) {
  model <- S7::prop(ColbertModelRef(value = list(model)), "value")[[1L]]
  query <- S7::prop(BebelScalarText(value = query), "value")
  if (!is.character(documents) || !length(documents) || anyNA(documents) || any(!nzchar(documents))) {
    stop("`documents` must be a non-empty character vector without missing or empty values.", call. = FALSE)
  }
  query_embeddings <- model$encode_query(query)
  scores <- vapply(documents, function(document) {
    query_embeddings$maxsim(model$encode_document(document))
  }, numeric(1))
  structure(sort(scores, decreasing = TRUE), class = c("colbertRanking", "numeric"))
}

#' @export
print.ColbertModel <- function(x, ...) {
  info <- x$info()
  cat("<ColbertModel>\n")
  cat("  profile: ", info$profile, "\n", sep = "")
  cat("  vectors: ", info$dimensions, " dimensions\n", sep = "")
  cat("  query positions: ", info$query_length, "\n", sep = "")
  cat("  document positions: ", info$document_length, "\n", sep = "")
  cat("  scoring: ", info$similarity, "\n", sep = "")
  invisible(x)
}

#' @export
print.ColbertEmbeddings <- function(x, ...) {
  info <- x$info()
  cat("<ColbertEmbeddings>\n")
  cat("  kind: ", info$kind, "\n", sep = "")
  cat("  tokens: ", info$tokens, "\n", sep = "")
  cat("  dimensions: ", info$dimensions, "\n", sep = "")
  cat("  row normalization: L2\n")
  invisible(x)
}

#' @export
print.colbertRanking <- function(x, ...) {
  cat("<ColBERT MaxSim ranking>\n")
  print(unclass(x), ...)
  invisible(x)
}
