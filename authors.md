# Authors and Citation

## Authors

- **Sounkou Mahamane Toure**. Author, maintainer.

- **Maxime Chevalier-Boisvert**. Copyright holder.  
  BebeLM upstream

- **Hiroaki Yutani**. Copyright holder.  
  savvy R/Rust interface

## Citation

Source:
[`DESCRIPTION`](https://github.com/sounkou-bioinfo/Rbebelm/blob/main/DESCRIPTION)

Toure S (2026). *Rbebelm: Native 'BebeLM' CPU Backend for R*. R package
version 0.3.6-0.1.0, <https://github.com/sounkou-bioinfo/Rbebelm>.

    @Manual{,
      title = {Rbebelm: Native 'BebeLM' CPU Backend for R},
      author = {Sounkou Mahamane Toure},
      year = {2026},
      note = {R package version 0.3.6-0.1.0},
      url = {https://github.com/sounkou-bioinfo/Rbebelm},
    }
