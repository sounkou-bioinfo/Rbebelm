...
Console commands include `/tools`, `/r`, `/transcript`, `/clear`, and
`/quit`. The `/r` command evaluates R directly in the same environment
used by the agent’s R tools. For example, `/r x <- mtcars` creates an
object that `r_objects()` can later see. Visible `/r` output is capped
so large objects do not flood the chat prompt; assign objects or use
summaries such as `/r str(x)` for inspection. The `r_eval` tool is only
